      ...
      <div class="flex flex-row items-center justify-between w-full gap-2 sm:gap-4">
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1 min-w-0">
            ...
            >
            Embassies - {{ country }} in {{ embassyName }}
            </h1>
...
const router = useRouter();
  ...
  const embassyName = computed(() => route.query.embassyName)
...
// Set page title
useHead({
  title: `${country.value} Embassies - iVisa`,
});

// Filter embassies by country
const embassies = computed(() => {
const countryEmbassies = allEmbassies.value.find(embassy => embassy.destinationCountry === country.value)
return countryEmbassies?.originCountries.find(originCountry => originCountry.name === embassyName.value)?.embassies || []
});
...
const filteredEmbassies = computed(() => {
  ...
  const embassyName = route.query.embassyName;
  const search = searchQuery.value.toLowerCase();

  // 1️⃣ Find the main embassy object by id
  const mainEmbassy = allEmbassies.value.find(
    (embassy) => embassy.id === Number(id)
  );
  if (!mainEmbassy) return [];

  // 2️⃣ Find the origin country inside it by embassyName
  const originCountry = mainEmbassy.originCountries.find(
    (origin) => origin.name.toLowerCase() === embassyName.toLowerCase()
  );
  if (!originCountry) return [];

  // 3️⃣ Filter its embassy list by searchQuery (if any)
  return originCountry.embassies.filter((embassy) =>
    embassy.name.toLowerCase().includes(search)
  );
});
...
const navigateToAddEmbassy = () => {
    router.push(`/dashboard/embassies/add?country=${country.value}`);
};

const viewEmbassy = (embassy) => {
  router.push(`/dashboard/embassies/add?id=${embassy.id}&mode=view`);
};

const editEmbassy = (embassy) => {
  // Find the destination country and origin country for this embassy
  const destinationCountry = allEmbassies.value.find(dest => 
    dest.originCountries.some(origin => 
      origin.embassies.some(e => e.name === embassy.name)
    )
  );
  
  const originCountry = destinationCountry?.originCountries.find(origin => 
    origin.embassies.some(e => e.name === embassy.name)
  );
  
  if (destinationCountry && originCountry) {
    router.push(
      `/dashboard/embassies/add?id=${destinationCountry.id}&mode=edit&destinationCountry=${encodeURIComponent(destinationCountry.destinationCountry)}&originCountry=${encodeURIComponent(originCountry.name)}&embassyName=${encodeURIComponent(embassy.name)}&address=${encodeURIComponent(embassy.location)}`
    );
  }
};
