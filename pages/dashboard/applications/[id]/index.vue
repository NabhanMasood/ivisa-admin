    ...
    <div class="flex-1">
      ...
      >
        <div class="space-y-6">
          ...
          <!-- Payment Link for Manual Applications -->
          <div
            v-if="application?.sourceType === 'manual' && application?.stripePaymentLinkUrl"
            class="bg-gray-50 dark:bg-gray-800/50 border border-orange-300 dark:border-orange-600 rounded-lg p-4"
          >
            <div class="flex items-center gap-2 mb-2">
              <svg class="w-5 h-5 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
              </svg>
              <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Stripe Payment Link</span>
              <span
                v-if="payment?.status === 'completed'"
                class="ml-auto px-2 py-0.5 text-xs font-medium rounded-full bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300"
              >
                Paid
              </span>
              <span
                v-else
                class="ml-auto px-2 py-0.5 text-xs font-medium rounded-full bg-yellow-100 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-300"
              >
                Awaiting Payment
              </span>
            </div>
            ...
            <div class="flex gap-2">
              <input
                :value="application?.stripePaymentLinkUrl"
                readonly
                class="flex-1 px-3 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-gray-700 dark:text-gray-300"
              />
              <button
              ...
              >
                <svg v-if="!paymentLinkCopied" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const copyPaymentLink = async () => {
  if (application.value?.stripePaymentLinkUrl) {
    try {
      await navigator.clipboard.writeText(application.value.stripePaymentLinkUrl);
